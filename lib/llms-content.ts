...
export async function buildLlmsContent({ includeFullContent, locale = 'en' }: LlmsOptions): Promise<string> {
  ...
  for (const group of docsConfig) {
    ...
    for (const item of group.items) {
      ...
      if (includeFullContent) {
        ...
        lines.push('---', '', `## ${doc.meta.title || item.titleKey}`, '', rawContent.trim(), '')
      } else {
        const description = doc.meta.description || ''
        lines.push(`- [${item.titleKey}](${localePrefix}/docs/${item.slug})${description ? `: ${description}` : ''}`)
      }
    }
    lines.push('')
  }
  ...
  if (repos.length > 0) {
    ...
    for (const repo of repos) {
      if (includeFullContent) {
        ...
        const readme = await getRepoReadme(repo.full_name, repo.default_branch, locale)
        if (readme) {
          lines.push('### README', '', readme.trim(), '')
        }
      } else {
        const description = repo.description || ''
        lines.push(`- [${repo.name}](${localePrefix}/projects/${repo.name})${description ? `: ${description}` : ''}`)
      }
    }
    lines.push('')
  }
  ...
  if (usesDoc) {
    if (includeFullContent) {
      ...
      lines.push(`## ${t.uses}`, '', `- [${t.usesLink}](${localePrefix}/uses): ${usesDoc.meta.description || t.usesDesc}`)
    }
  }
